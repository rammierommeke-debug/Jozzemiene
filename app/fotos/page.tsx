...
export default function FotosPage() {
  ...
  async function handleUpload(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    for (let i = 0; i < files.length; i++) {
      ...
      // 2. Upload the file directly to Supabase Storage (bypasses Vercel size limit)
      ...
      if (!uploadRes.ok) {
        console.error("Upload to Supabase failed:", uploadRes.status);
        setUploadProgress({ done: i + 1, total: files.length });
        continue;
      }

      // 3. Save the DB record via our API (JSON, no file)
  ...
  async function saveCaption() {
    if (!lightbox) return;
    const res = await fetch(`/api/photos/${lightbox.id}`, {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
  ...
  return (
    <div className="max-w-4xl mx-auto pt-14 md:pt-0">
        ...
        <div className="flex gap-2">
          {selectMode ? (
            ...
            <>
              {photos.length > 0 && (
                <a
                  href="/api/photos/download?album_id=all"
                  download
      ...
      {showAlbumForm && (
        <div className="bg-rose-light/30 rounded-3xl p-5 border border-warm mb-6">
          ...
          <div className="flex gap-2 mb-3">
            <input value={newAlbumEmoji} onChange={(e) => setNewAlbumEmoji(e.target.value)} className="w-12 bg-cream rounded-xl border border-warm text-center py-2 focus:outline-none focus:border-rose" maxLength={2} />
            <input value={newAlbumName} onChange={(e) => setNewAlbumName(e.target.value)} onKeyDown={(e) => e.key === "Enter" && createAlbum()} placeholder="Naam van het album (bv. Parijs 2024)" className="flex-1 bg-cream rounded-xl border border-warm px-4 py-2 text-sm text-brown focus:outline-none focus:border-rose" />
          </div>
          <div className="flex gap-2">
      ...
      {!selectMode && (
        <div className="bg-warm rounded-3xl p-6 border border-warm mb-8">
          ...
          <div className="flex flex-col gap-3">
            <input value={caption} onChange={(e) => setCaption(e.target.value)} placeholder="Bijschrift toevoegen..." className="bg-cream rounded-xl border border-warm px-4 py-2 text-sm text-brown focus:outline-none focus:border-rose" />
            <div className="flex gap-2 flex-wrap">
              <button onClick={() => setSelectedAlbum(null)} className={`px-3 py-1 rounded-full text-xs font-semibold transition-all ${!selectedAlbum ? "bg-rose text-cream" : "bg-cream text-brown-light hover:bg-rose/20"}`}>Geen album</button>
              {albums.map((a) => (
                <button key={a.id} onClick={() => setSelectedAlbum(a.id)} className={`px-3 py-1 rounded-full text-xs font-semibold transition-all ${selectedAlbum === a.id ? "bg-rose text-cream" : "bg-cream text-brown-light hover:bg-rose/20"}`}>{a.emoji} {a.name}</button>
              ))}
            </div>
            <div className="flex flex-col gap-2">
              <input ref={fileRef} type="file" accept="image/*" multiple onChange={handleUpload} className="hidden" />
              <button onClick={() => fileRef.current?.click()} disabled={uploading} className="flex items-center gap-2 bg-rose text-cream px-5 py-2.5 rounded-2xl text-sm font-semibold hover:bg-rose/80 transition-colors disabled:opacity-50">
                <Upload size={16} />
              ...
              {uploading && uploadProgress && (
                <div className="w-full bg-cream rounded-full h-1.5 overflow-hidden">
                  <div className="bg-rose h-1.5 rounded-full transition-all duration-300" style={{ width: `${(uploadProgress.done / uploadProgress.total) * 100}%` }} />
                </div>
              )}
            </div>
          </div>
      ...
      {loading ? (
        ...
        <div className="flex flex-col gap-6">
          {albums.map((album) => {
            ...
            return (
              <div key={album.id} className="bg-cream rounded-3xl border border-warm overflow-hidden">
                <button onClick={() => toggleExpand(album.id)} className="w-full flex items-center gap-3 p-5 hover:bg-warm/30 transition-colors group">
                  <span className="text-xl">{album.emoji}</span>
                  <div className="flex-1 text-left">
                    <p className="font-display text-lg text-brown">{album.name}</p>
                    <p className="text-xs text-brown-light">{albumPhotos.length} foto{albumPhotos.length !== 1 ? "'s" : ""}</p>
                  </div>
                  <div className="flex items-center gap-2">
                    {albumPhotos.length > 0 && (
                      <a
                        href={`/api/photos/download?album_id=${album.id}`}
                        download
                        ...
                        title="Download als zip"
                      >
                        <Download size={14} />
                      </a>
                    )}
                    <button onClick={(e) => { e.stopPropagation(); deleteAlbum(album.id); }} className="opacity-0 group-hover:opacity-100 text-rose transition-opacity p-1"><Trash2 size={14} /></button>
                ...
                {expanded && (
                  <div className="px-5 pb-5">
                    {albumPhotos.length === 0 ? (
                      <p className="text-sm text-brown-light italic text-center py-4">Nog geen foto's in dit album 🌸</p>
                    ) : (
                      <div className="columns-2 sm:columns-3 gap-3 space-y-3">
                        {albumPhotos.map((photo) => (
                          <PhotoCard key={photo.id} photo={photo} onOpen={openLightbox} selectMode={selectMode} selected={selected.has(photo.id)} onToggleSelect={toggleSelect} />
                        ))}
                      </div>
                    )}
                  </div>
          ...
          {unassigned.length > 0 && (
            <div className="bg-cream rounded-3xl border border-warm overflow-hidden">
              <button onClick={() => toggleExpand("__none__")} className="w-full flex items-center gap-3 p-5 hover:bg-warm/30 transition-colors group">
                <FolderOpen size={20} className="text-brown-light" />
                <div className="flex-1 text-left">
                  <p className="font-display text-lg text-brown">Overige foto's</p>
                  <p className="text-xs text-brown-light">{unassigned.length} foto{unassigned.length !== 1 ? "'s" : ""}</p>
                </div>
                <div className="flex items-center gap-2">
                  <a
                    href="/api/photos/download?album_id=none"
                    download
                    onClick={(e) => e.stopPropagation()}
                    className="opacity-0 group-hover:opacity-100 flex items-center gap-1 text-xs text-sage font-semibold hover:text-sage/70 transition-all p-1"
                    title="Download als zip"
      ...
      {/* Lightbox */}
      {lightbox && !selectMode && (
        <div className="fixed inset-0 bg-brown/70 z-50 flex items-center justify-center p-6" onClick={() => { setLightbox(null); setEditingCaption(false); }}>
          <div className="relative max-w-2xl w-full bg-cream rounded-3xl overflow-hidden shadow-xl" onClick={(e) => e.stopPropagation()}>
            <NextImage src={lightbox.url} alt={lightbox.caption || ""} width={800} height={600} className="w-full object-contain max-h-[65vh]" />
              ...
              {editingCaption ? (
                <div className="flex-1 flex gap-2 items-start">
                  <textarea
                    ...
                    placeholder="Schrijf een berichtje bij deze foto..."
                    autoFocus
                    rows={2}
                  ...
                  <div className="flex flex-col gap-1">
                    <button onClick={saveCaption} className="bg-sage text-cream rounded-xl p-2 hover:bg-sage/80 transition-colors" title="Opslaan">
                      <Check size={15} />
                    </button>
                    <button onClick={() => { setEditingCaption(false); setCaptionDraft(lightbox.caption || ""); }} className="bg-warm text-brown-light rounded-xl p-2 hover:bg-warm/80 transition-colors" title="Annuleren">
                      <X size={15} />
                    </button>
                  </div>
                </div>
                ...
                <div className="flex-1 flex items-center gap-2 group/caption">
                  ...
                  <button onClick={() => setEditingCaption(true)} className="opacity-0 group-hover/caption:opacity-100 transition-opacity text-brown-light hover:text-terracotta">
                    <Pencil size={14} />
                  </button>
                </div>
              )}
            ...
            <div className="absolute top-3 right-3 flex gap-2">
              <button onClick={() => deletePhoto(lightbox.id)} className="bg-rose/90 text-cream rounded-xl p-2 hover:bg-rose transition-colors"><Trash2 size={16} /></button>
              <button onClick={() => { setLightbox(null); setEditingCaption(false); }} className="bg-brown/80 text-cream rounded-xl p-2 hover:bg-brown transition-colors"><X size={16} /></button>
            </div>
          </div>
        </div>
      )}
...
function PhotoCard({ photo, onOpen, selectMode, selected, onToggleSelect }: {
  photo: Photo;
  onOpen: (p: Photo) => void;
  selectMode: boolean;
  selected: boolean;
  ...
  return (
    <div
      className={`relative group rounded-2xl overflow-hidden cursor-pointer break-inside-avoid transition-all ${selected ? "ring-3 ring-rose scale-[0.97]" : ""}`}
      onClick={() => selectMode ? onToggleSelect(photo.id) : onOpen(photo)}
    >
      <NextImage src={photo.url} alt={photo.caption || ""} width={400} height={400} className="w-full object-cover transition-transform duration-300 group-hover:scale-105" />
      <div className={`absolute inset-0 transition-all duration-200 ${selected ? "bg-rose/30" : "bg-brown/0 group-hover:bg-brown/20"}`} />
      {/* Selectie checkbox */}
      {selectMode && (
        <div className="absolute top-2 left-2">
          {selected
            ? <CheckSquare size={20} className="text-rose drop-shadow" />
