...
import { PiggyBank, Plus, Trash2, Euro, TrendingUp, Check } from "lucide-react";
...
function buildMonthlyData(goals: Goal[]) {
  ...
  const entries = Object.entries(map).slice(-6);
...
export default function SparenPage() {
  ...
  function addSavings(id: string) {
    ...
    update(goals.map((g) =>
      g.id === id
        ? { ...g, entries: [...g.entries, { amount, date: new Date().toISOString() }] }
        : g
    ));
    setAddingTo(null);
    setAddAmount("");
    ...
    <div className="max-w-2xl mx-auto pt-14 md:pt-0">
      ...
      <div className="flex gap-2 mb-6 bg-warm rounded-2xl p-1">
        {(["doelen", "overzicht"] as const).map((t) => (
          ...
          >
            {t === "doelen" ? "🎯 Doelen" : "📊 Maandoverzicht"}
          </button>
        ))}
      </div>
        ...
        <div className="flex flex-col gap-5">
          ...
          <div className="bg-cream rounded-3xl p-5 border border-warm">
            ...
            {monthlyData.length === 0 ? (
              <p className="text-brown-light font-handwriting text-lg text-center py-8">Nog geen stortingen 🌱</p>
            ) : (
